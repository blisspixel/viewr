...
| Item | State |
| --- | --- |
| Published install target | Immutable [v0.6.4](https://github.com/blisspixel/viewr/releases/tag/v0.6.4), the shipped-feedback patch over v0.6.3, published without the carried representative-hardware matrix |
| Workspace version | `0.6.4` while the carried v0.6 hardware gate remains open. A non-publishing candidate is identified by its full commit SHA and workflow run, not by treating its embedded version as a new public release |
...
| Next version update | After the v0.6 hardware gate closes, one reviewed release-preparation change advances the workspace and release documents from `0.6.4` to `0.7.0` before v0.7 evidence is collected |
...
but publisher-unsigned v0.6.4 archives. The v0.6.3, v0.6.2, v0.6.1, v0.6.0, v0.5.0, v0.4.0, v0.3.0,
...
v0.6.3  Localization coverage patch      [released, same matrix open]
   |
   v
v0.6.4  Network storage & culling patch  [released, same matrix open]
   |
   v
...
| --- | --- | --- | --- |
| Public release | `0.6.4` | Immutable tag and release assets | Never rewritten. A shipped correction uses a new v0.6.x tag |
| Carried v0.6 evidence candidate | `0.6.4` | Full commit SHA plus one `Release artifacts` run | Replaced after any candidate-invalidating change |
...
v0.6.4; v0.1.0 remains the first immutable preview. Reliability, fidelity,
...
- [x] Make the release-state and quality-gate contract executable: canonical
  documentation now agrees on the public unsigned v0.6.4 state, CI runs the exact
...
- [ ] Exercise first-time, fast-path, admin, failure-recovery, keyboard-only, and
  ...
  v0.6.1, v0.6.2, v0.6.3, and v0.6.4 were tagged and published before this box could be checked, so the
