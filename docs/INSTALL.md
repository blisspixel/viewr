...
viewr is pre-1.0. v0.6.5 is the current public GitHub Release, a shipped-feedback
patch over v0.6.4 and the integrated v0.6.0 product-quality beta. Its portable
...
the [v0.6.5 notes](releases/v0.6.5.md) record that gap exactly.
...
```powershell
irm https://github.com/blisspixel/viewr/releases/download/v0.6.5/install.ps1 `
...
above are fixed to the immutable `v0.6.5` release rather than a moving branch.
To review it first:

```sh
curl -fsSLO https://github.com/blisspixel/viewr/releases/download/v0.6.5/install.sh
less install.sh
sh install.sh
```
...
GitHub checksums and attestations improve integrity and provenance. The v0.6.5
