...
the security policy, canonical Markdown, and the bounded PNG interface captures used
...
successful manual release workflow, branch, and commit, then the validator
downloads that exact run into a fresh temporary directory. It applies the
canonical archive verifier to all four archives, checks the record digests,
binds every retained performance report to both applicable archived executables,
and requires the exact synthetic fixture artifact generated by that run,
including its canonical per-file checksum manifest. A prepopulated local
directory is not accepted as workflow provenance.

PQ-VS-04 also requires the extracted candidate binary to pass the complete GUI
performance harness on each named representative host or session. Retain a new,
path-free JSON report for every execution:

```text
python -B scripts/performance_gate.py --binary <extracted-candidate-binary> --no-xvfb --idle-diagnostics --session-label <session> --report-file docs/release-evidence/product-quality/v0.6.5/performance/<session>.json
```

The report binds its numeric runs and result to the SHA-256 values of both shipped
executables, removes any inherited decoder-worker override, runs private copies,
records measured display or Linux session facts plus the actual wgpu adapter used
by every probe, and refuses to overwrite earlier evidence. The complete
eight-report set is committed beside the platform records. The product-quality
record carries the recomputed worst-case rollup. See [Product
quality](PRODUCT-QUALITY.md) for the exact session labels, archive `bin/` paths,
display procedure, and Linux software-renderer controls.
