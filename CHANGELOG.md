...
- Ephemeral verification workspaces now reject empty, overlong, non-ASCII, and
  path-like diagnostic labels before touching the filesystem. Labels never enter
  the on-disk path, and each unique directory is verified as an immediate child
  of the process temp root.
...
- The first window fits the monitor it opens on. viewr asked for a 1000 by 720
  ...
  bounded by the monitor, at most 90 percent of its logical width and 70 percent
  of its height and never below the 640 by 480 minimum, and the window is placed
  5 percent of the monitor height below its top edge, so the bottom fifth of the
  screen stays free for the furniture. Larger displays keep the same size, no
  window changes size while an image loads, and Wayland, which does not accept
  client placement, still gets the bounded size.
