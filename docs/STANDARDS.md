...
preview-identity policy. The private `curation_state` seam adds operation,
recovery-priority, status, count, and deferred-close policy without creating a
second mutable application owner. Coverage is a floor and a signal, not the goal.
The real goal is that the tests would catch a regression, which is why we also
