...
viewr is pre-1.0. v0.6.2 is the current public GitHub Release, a shipped-feedback
patch over v0.6.1 and the integrated v0.6.0 product-quality beta. Its portable
...
the [v0.6.2 notes](releases/v0.6.2.md) record that gap exactly.
...
```powershell
irm https://github.com/blisspixel/viewr/releases/download/v0.6.2/install.ps1 | iex
```
...
```powershell
irm https://github.com/blisspixel/viewr/releases/download/v0.6.2/install.ps1 `
  -OutFile $env:TEMP\viewr-install.ps1
& $env:TEMP\viewr-install.ps1 -Version 0.6.1
```
...
```sh
curl -fsSL https://github.com/blisspixel/viewr/releases/download/v0.6.2/install.sh | sh
```
...
above are fixed to the immutable `v0.6.2` release rather than a moving branch.
To review it first:

```sh
curl -fsSLO https://github.com/blisspixel/viewr/releases/download/v0.6.2/install.sh
less install.sh
sh install.sh
```
...
GitHub checksums and attestations improve integrity and provenance. The v0.6.2
