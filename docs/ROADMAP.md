...
| Display correctness | Partial for v0.3 | Embedded RGB profiles normalize into the bounded sRGB path; sRGB, Display P3, and Adobe RGB reference vectors plus CMYK fallback are tested; worker Display P3 CICP converts instead of falling back; the window's monitor identity is observed and refreshed; Linux window-system class is mapped onto the color policy so unmanaged X11 reports the sRGB fallback; Windows ICM profile bytes are admitted only on unmanaged paths; Linux X11 atom lookup, per-display transforms, wide-gamut, and HDR remain. |
...
- [ ] Resolve and refresh the profile for the display that currently contains the
  ...
  bytes are read when that policy would record them; Linux X11 `_ICC_PROFILE`
  lookup is still open. In-app pixel application is deferred until a
