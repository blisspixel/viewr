...
use crate::{
    ...
    ciexyz::*, consts::*, icc::try_colour_encoding_to_icc, tf,
};
...
impl ColorTransform {
    ...
    fn with_builder<Cms: ColorManagementSystem + ?Sized>(
        ...
        validate_transform_encodings(from, to)?;
            ...
            ColourEncoding::IccProfile(_) => {
                ...
                match &to.encoding {
                    ...
                    ColourEncoding::Enum(encoding) => {
                        ...
                        let from = &from.icc_profile;
                        let to = try_colour_encoding_to_icc(&target_encoding)?;
                        let transform = cms
                            .prepare_transform(from, &to, rendering_intent)
                            .map_err(crate::Error::CmsFailure)?;
        ...
        let target_encoding = match &to.encoding {
            ...
            ColourEncoding::IccProfile(_) => {
                let from = try_colour_encoding_to_icc(&current_encoding)?;
                let to = &to.icc_profile;
                let transform = cms
...
fn enum_encoding_is_transformable(encoding: &EnumColourEncoding) -> bool {
    if encoding.colour_space == ColourSpace::Xyb {
        return true;
    }
    if encoding.colour_space == ColourSpace::Unknown
        || encoding.tf == TransferFunction::Unknown
        || matches!(encoding.tf, TransferFunction::Gamma { g: 0, .. })
    {
        return false;
    }
    encoding.colour_space != ColourSpace::Grey || encoding.tf != TransferFunction::Hlg
}

fn validate_transform_encodings(
    from: &ColorEncodingWithProfile,
    to: &ColorEncodingWithProfile,
) -> Result<()> {
    for encoding in [&from.encoding, &to.encoding] {
        if let ColourEncoding::Enum(encoding) = encoding
            && !enum_encoding_is_transformable(encoding)
        {
            return Err(Error::UnsupportedColorEncoding);
        }
    }
    ...
    fn run(&self, channels: &mut [&mut [f32]], num_input_channels: usize) -> Result<usize> {
        ...
        Ok(match self {
            ...
            Self::HlgInverseOotf(HdrParams {
                ...
                // HLG inverse OOTF is an RGB operation. Transform construction
                // guarantees three channels before this operation is emitted.
...
mod tests {
    ...
    fn grey_hlg() -> ColorEncodingWithProfile {
        ...
        encoding.colour_space = ColourSpace::Grey;
        encoding.tf = TransferFunction::Hlg;
        ColorEncodingWithProfile::new(encoding)
    }

    fn assert_unsupported_transform_pair(
        from: &ColorEncodingWithProfile,
        to: &ColorEncodingWithProfile,
    ) {
        assert!(matches!(
    ...
    fn grayscale_hlg_is_rejected_in_both_transform_directions() {
        ...
        assert_unsupported_transform_pair(&grey_hlg, &srgb);
        assert_unsupported_transform_pair(&grey_hlg, &grey_hlg);
    }

    #[test]
    fn hlg_transfer_helpers_reject_a_single_channel() {
        ...
        assert!(matches!(
            apply_transfer_function(&mut channels, TransferFunction::Hlg, params),
            Err(Error::UnsupportedColorEncoding)
        ));
        assert!(matches!(
